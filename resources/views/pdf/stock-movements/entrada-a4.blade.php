...
<head>
    ...
    <title>Comprobante de Entrada de Mercaderia - {{ $movement->document_number }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        .stock-table td.entry .stock-value {
            color: #27ae60;
        }
...
<body>
    <div class="header">
        <table class="header-table">
            <tr>
                ...
                <td class="document">
                    <span class="document-type">ENTRADA DE MERCADERIA</span><br>
    ...
    <div class="section">
        ...
        <table class="stock-table">
            <tr>
                ...
                <td class="entry">
                    <span class="stock-label">Entrada</span>
    ...
    <div class="section">
        ...
        <table class="details-table">
            <tr>
                ...
                <td>
                    <div class="info-label">{{ $movement->supplier_id && $movement->supplier ? 'Proveedor' : '—' }}</div>
                    <div class="info-value">
                        @if($movement->supplier_id && $movement->supplier)
                            {{ $movement->supplier->name }}
                            @if($movement->supplier->cuit)
                                <br><span style="font-size: 7pt; color: #7f8c8d;">{{ $movement->supplier->cuit }}</span>
                            @endif
                        @else
                            —
                        @endif
                ...
                <td>
                    <div class="info-label">{{ $movement->invoice_reference ? 'Factura / Guía' : '—' }}</div>
                    <div class="info-value">{{ $movement->invoice_reference ?? '—' }}</div>
                </td>
            </tr>
            <tr>
                <td>
                    <div class="info-label">{{ $movement->batch_number ? 'Lote' : '—' }}</div>
                    <div class="info-value">{{ $movement->batch_number ?? '—' }}</div>
                </td>
                <td>
                    <div class="info-label">{{ $movement->expiry_date ? 'Vencimiento' : '—' }}</div>
                    <div class="info-value">{{ $movement->expiry_date ? $movement->expiry_date->format('d/m/Y') : '—' }}</div>
                </td>
                <td>
    ...
    <table class="signature-table">
        <tr>
            <td>
                <span class="signature-label">Firma del Responsable</span>
                <span class="signature-role">{{ $movement->user_name }}</span>
            </td>
            <td>
                <span class="signature-label">Firma y Sello Autorizado</span>
                <span class="signature-role">Administrador / Gerente</span>
            </td>
        </tr>
    </table>
