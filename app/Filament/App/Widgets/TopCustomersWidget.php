            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('last_purchase')
                    ...
                    ->color(function ($record) {
                        if (!$record->last_purchase) return 'gray';
                        
                        $daysAgo = now()->diffInDays($record->last_purchase);
                        
                        if ($daysAgo <= 7) return 'success';
                        if ($daysAgo <= 15) return 'warning';
                        return 'danger';
                    })
                    ->icon(function ($record) {
                        if (!$record->last_purchase) return null;
                        
                        $daysAgo = now()->diffInDays($record->last_purchase);
                        
                        if ($daysAgo <= 7) return 'heroicon-o-check-circle';
                        if ($daysAgo <= 15) return 'heroicon-o-clock';
