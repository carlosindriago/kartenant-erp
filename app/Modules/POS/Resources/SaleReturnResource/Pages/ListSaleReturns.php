    ...
    public function getTabs(): array
    {
        return [
            'todas' => Tab::make('Todas')
                ->badge(fn () => SaleReturn::count()),

            'hoy' => Tab::make('Hoy')
                ->modifyQueryUsing(fn (Builder $query) => $query->whereDate('created_at', today()))
                ->badge(fn () => SaleReturn::whereDate('created_at', today())->count())
                ->badgeColor('warning'),

            'completadas' => Tab::make('Completadas')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('status', 'completed'))
                ->badge(fn () => SaleReturn::where('status', 'completed')->count())
                ->badgeColor('success'),

            'completas' => Tab::make('Devoluciones Completas')
                ->modifyQueryUsing(fn (Builder $query) => $query->where('return_type', 'full'))
                ->badge(fn () => SaleReturn::where('return_type', 'full')->count())
                ->badgeColor('warning'),
        ];
    }
}
