        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('reference')
                    ...
                    ->description(fn (StockMovement $record): ?string => $record->reference ? 'Click para ver detalle' : null
                    )
                    ->tooltip(function (StockMovement $record): ?string {
                        if (! $record->reference) {
                            return null;
                    ...
                    ->url(function (StockMovement $record): ?string {
                        ...
                        if (str_contains($record->reference, 'NCR:')) {
                            ...
                            if (isset($matches[1])) {
                                ...
                                $saleReturn = SaleReturn::where('return_number', $returnNumber)->first();
                ...
                Tables\Columns\TextColumn::make('user_name')
                    ...
                    ->description(fn (StockMovement $record): ?string => $record->user_name && $record->user_name !== 'Sistema'
                            ? 'Click para ver perfil'
                            : null
                    )
                    ->tooltip(fn (StockMovement $record): ?string => $record->user_name && $record->user_name !== 'Sistema'
