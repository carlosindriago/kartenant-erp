...
use App\Console\Commands\ManageStoreSettings;
...
if ($__allowMakeSuperadmin) {
    Artisan::command('kartenant:make-superadmin {email?} {--name=} {--password=}', function () {
        ...
        try {
            ...
            $allPermModels = Permission::query()->get();
            ...
            Log::critical('Comando kartenant:make-superadmin ejecutado', [
                'env' => config('app.env'),
                'executor' => $execUser,
                'host' => $host,
                'email' => $email,
            ...
            if (config('logging.channels.slack.url')) {
                Log::channel('slack')->critical('Kartenant: SUPERADMIN creado/promovido', [
                    'env' => config('app.env'),
                    'executor' => $execUser,
                    'host' => $host,
                    'email' => $email,
            ...
            Log::critical('Fallo kartenant:make-superadmin', [
                'env' => config('app.env'),
...
if ($__allowDeleteSuperadmin) {
    Artisan::command('kartenant:delete-superadmin {email?} {--force}', function () {
        ...
        try {
            ...
            // Log de seguridad
            Log::critical('Super Admin eliminado', [
                'env' => config('app.env'),
                'email' => $userEmail,
                'name' => $userName,
                'timestamp' => now()->toDateTimeString(),
...
Artisan::command('emporio:store-settings {action : Action to perform (migrate, seed, reset)} {--tenant= : Specific tenant ID (optional)} {--force : Force execution without confirmation}', function () {
    ...
    $command = new ManageStoreSettings;
